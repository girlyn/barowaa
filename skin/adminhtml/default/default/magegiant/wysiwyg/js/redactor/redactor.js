...
Redactor.prototype = {
        ...
        $mgt.each(this.opts.buttons, $mgt.proxy(function (i, key) {
            ...
            if (key !== '|' && typeof this.opts.toolbar[key] !== 'undefined') {
                ...
                if (key === 'backcolor' || key === 'fontcolor' || typeof(s.dropdown) !== 'undefined') {
                    ...
                    // observing dropdown
                    this.hdlHideDropDown = $mgt.proxy(function (e) {
                        this.hideDropDown(e, dropdown, key);
                    }, this);
                    this.hdlShowDropDown = $mgt.proxy(function (e) {
    ...
    buildButton: function (key, s) {
        ...
        if (typeof s.func === 'undefined') {
            button.click($mgt.proxy(function () {
            ...
            function (x, d) {
                ...
                if (typeof d.name !== 'undefined' && d.name === 'separator') {
                    ...
                    if (typeof(d.func) === 'undefined') {
                        $mgt(drop_a).click($mgt.proxy(function () {
                            this.execCommand(d.exec, x);
                        }, this));
                    }
                    else {
                        $mgt(drop_a).click($mgt.proxy(function (e) {
                            this[d.func](e);
                        }, this));
                    }
                }
    ...
    resizeImage: function (resize) {
        ...
        $mgt(resize).hover(function () {
            $mgt(resize).css('cursor', 'nw-resize');
        }, function () {
            $mgt(resize).css('cursor', 'default');
            clicked = false;
        });
    ...
    imageEdit: function (e) {
        ...
        var handler = $mgt.proxy(function () {
            ...
            $mgt('#redactor_image_delete_btn').click($mgt.proxy(function () {
                this.imageDelete($mgtel);
            }, this));
            $mgt('#redactorSaveBtn').click($mgt.proxy(function () {
                this.imageSave($mgtel);
            }, this));
    ...
    modalInit: function (title, url, width, handler, endCallback) {
        ...
        this.hdlModalClose = $mgt.proxy(function (e) {
            if (e.keyCode === 27) {
                this.modalClose();
            }
        }, this);
    ...
    Construct.prototype = {
        init: function () {
            if (!$mgt.browser.msie) {
                ...
                // drag over
                this.dropareabox.bind('dragover', $mgt.proxy(function () {
                    return this.ondrag();
                }, this));

                // drag leave
                this.dropareabox.bind('dragleave', $mgt.proxy(function () {
 ...
 */
(function ($mgt) {
    $mgt.event.special.textselect = {setup: function (data, namespaces) {
        $mgt(this).data("textselected", false);
        $mgt(this).data("ttt", data);
        $mgt(this).bind('mouseup', $mgt.event.special.textselect.handler)
    }, teardown: function (data) {
        $mgt(this).unbind('mouseup', $mgt.event.special.textselect.handler)
    }, handler: function (event) {
        var data = $mgt(this).data("ttt");
        var text = $mgt.event.special.textselect.getSelectedText(data).toString();
        if (text != '') {
            $mgt(this).data("textselected", true);
            event.type = "textselect";
            event.text = text;
            $mgt.event.handle.apply(this, arguments)
        }
    ...
    $mgt.event.special.textunselect = {setup: function (data, namespaces) {
        $mgt(this).data("rttt", data);
        $mgt(this).data("textselected", false);
        $mgt(this).bind('mouseup', $mgt.event.special.textunselect.handler);
        $mgt(this).bind('keyup', $mgt.event.special.textunselect.handlerKey)
    }, teardown: function (data) {
        $mgt(this).unbind('mouseup', $mgt.event.special.textunselect.handler)
    }, handler: function (event) {
        if ($mgt(this).data("textselected")) {
            var data = $mgt(this).data("rttt");
            var text = $mgt.event.special.textselect.getSelectedText(data).toString();
            if (text == '') {
                $mgt(this).data("textselected", false);
